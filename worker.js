...
var src_default = {
  async fetch(request, env) {
    ...
    const frontendUrl =
      'https://raw.githubusercontent.com/bulianglin/psub/main/frontend.html';
    ...
    replacedString =
      ...
      urlSafeBase64Encode(
        link
          .replace(
            password,
            urlSafeBase64Encode(replacements[urlSafeBase64Decode(password)])
          )
...
function replaceSS(link, replacements, isRecovery) {
  ...
  if (tempLink.includes('@')) {
    ...
    if (isRecovery) {
      const newStr = urlSafeBase64Encode(
        encryption + ':' + replacements[password]
      );
      replacedString = link
